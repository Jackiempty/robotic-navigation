...
class YoloDetectionNode(Node):
    def __init__(self):
        ...
        model_path = os.path.join(
            get_package_share_directory("yolo_example_pkg"), "models", "det_best.pt"
        )
    ...
    def draw_bounding_boxes(self, image, results):
        ...
        for result in results:
            for box in result.boxes:
                ...
                # 計算 y_f (左側為正，右側為負，轉換成一個合理比例)
                img_width = image.shape[1]
                y_f = -float(delta_x) / (img_width / 2.0) # range is roughly -1 to 1

                yolo_data.append({
                    "label": class_name,
                    "offset_flu": [float(target_distance), y_f, 0.0]
                })

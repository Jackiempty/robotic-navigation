...
class RosCommunicator(Node):
    ...
    def subscriber_cmd_vel_callback(self, msg):
        ...
        # 若完全沒有速度指令，強制發布 0 讓車子煞車
        if abs(v) < 0.01 and abs(omega) < 0.01:
            self.publish_raw_car_control([0.0, 0.0, 0.0, 0.0])
            return

        # 這裡將數值放大，因為 cmd_vel 的數值通常很小，但 Unity 內的車輪需要較大的 rad/s
        # 恢復預設的 600 倍率避免翻車
        multiplier = 600.0
        v_left *= multiplier
        v_right *= multiplier

        # 加入靜摩擦力補償 (Deadband Compensation)
        # 為了克服原地轉向時巨大的橫向靜摩擦力，我們將摩擦力門檻稍微提高
        min_power = 220.0  
        max_speed = max(abs(v_left), abs(v_right))
        
        if max_speed > 0:
            ratio = (max_speed + min_power) / max_speed
            v_left *= ratio
            v_right *= ratio

        # 限制最大輸出 (Clamp)
        # 先前翻車是因為瞬間輸出可能飆破 2000。我們將最高極限鎖定在 450，
        # 並且依照等比例縮小，確保既不會翻車，也能完美保持左右輪的差速比例！
        MAX_POWER = 450.0
        current_max = max(abs(v_left), abs(v_right))
        if current_max > MAX_POWER:
            clamp_ratio = MAX_POWER / current_max
            v_left *= clamp_ratio
            v_right *= clamp_ratio

        self.publish_raw_car_control([v_left, v_right, v_left, v_right])

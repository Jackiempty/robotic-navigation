...
class ArmController:
    ...
    def pounce_action(self):
        """執行往前撲取抓熊！ (寫死的相對動作)"""
        print("執行往前撲取動作...")
        # 直接呼叫現有的抓取流程，假設目標在正前方約 25 公分，高度 0
        self._execute_grab_sequence(0.25, 0.0)

    def move_end_effector(self, x_offset, y_offset, z_offset):
        """移動末端點到指定相對位置 (推門把)"""
        print(f"移動末端點推門: x={x_offset}, z={z_offset}")
        # 打開夾爪，往前伸直
        target_angles = [None, None, self.joint_limits[2]["max_angle"]]
        self._smooth_move_to(target_angles, step=5.0, delay=0.1)
        
        # IK 計算往前伸
        deg1, deg2 = self._calculate_2d_ik(x_offset, z_offset)
        self._smooth_move_to([deg1, deg2, None], step=5.0, delay=0.1)
        time.sleep(1.0)